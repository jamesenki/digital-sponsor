...
# Simple deployment of Digital Sponsor Services using Azure Container Instances
# Uses inline Node.js applications for quick deployment
...
app.post(\"/api/chat\", async (req, res) => {
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: \"system\", content: \"You are a supportive AI assistant for Digital Sponsor platform, helping individuals in recovery using AA principles. Be compassionate and encouraging.\" },
...
echo "📚 Deploying Literature Service..."
...
const mockLiterature = [
  { 
    ...
    content: \"We admitted we were powerless over alcohol—that our lives had become unmanageable. This is the first step in recovery, acknowledging our powerlessness.\",
    type: \"twelve_steps\", 
    step: 1,
    relevanceScore: 0.95
  },
  { 
    ...
    content: \"Made a searching and fearless moral inventory of ourselves. This step involves honest self-examination and writing down our resentments, fears, and sexual conduct.\",
    type: \"twelve_steps\", 
    step: 4,
    relevanceScore: 0.88
  },
  { 
    ...
    content: \"Our common welfare should come first; personal recovery depends upon A.A. unity. Unity is the cornerstone of our fellowship.\",
...
app.post(\"/api/search\", async (req, res) => {
  try {
    ...
    let results = mockLiterature.filter(item => {
      const searchText = (item.title + \" \" + item.content).toLowerCase();
      return lowerQuery.split(\" \").some(term => searchText.includes(term));
    });

    if (results.length === 0) results = mockLiterature.slice(0, maxResults);
    else results = results.slice(0, maxResults);
...
app.listen(3002, \"0.0.0.0\", () => {
  console.log(\"📚 Literature Service running on port 3002\");
...
echo "📚 Literature Service Status:"  
...
echo "💬 Chat Service:"
echo "   - Health: http://digitalsponsor-chat-prod.eastus2.azurecontainer.io:3003/health"
echo "   - API: http://digitalsponsor-chat-prod.eastus2.azurecontainer.io:3003/api/chat"
echo ""
echo "📚 Literature Service:"
echo "   - Health: http://digitalsponsor-literature-prod.eastus2.azurecontainer.io:3002/health"  
echo "   - Search: http://digitalsponsor-literature-prod.eastus2.azurecontainer.io:3002/api/search"
echo "   - Embeddings: http://digitalsponsor-literature-prod.eastus2.azurecontainer.io:3002/api/embed"
echo ""
echo "🧪 Test Commands:"
...
echo "  -d '{\"message\": \"I need encouragement for my recovery today\"}'"